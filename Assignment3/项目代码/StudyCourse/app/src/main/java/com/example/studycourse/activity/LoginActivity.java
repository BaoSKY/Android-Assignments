...
import com.example.studycourse.helper.MyDatabaseHelper;
...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //实例化MyDataHelper
        databaseHelper = new MyDatabaseHelper(this, MyDatabaseHelper.DATABASE_NAME, null, 1);
    ...
    private void login(){
        ...
        SQLiteDatabase database = databaseHelper.getWritableDatabase();
        Cursor cursor = database.rawQuery("SELECT mail FROM user", null);
        boolean exist = false;
        if(cursor.moveToFirst()){
            do{
                if(user.equals(cursor.getString(cursor.getColumnIndex("mail")))){
                    exist = true;
                    break;
                }
        ...
        if(!exist){
            database.execSQL("INSERT INTO user (mail, password) VALUES (?, ?)",
